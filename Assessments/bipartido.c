...
int ehbipartido(TGrafo *g) {    
    ...
    while (vertice != NULL) {
        if (vertice->cor == -1) {
            push(pilha, vertice->id_vertice);
            vertice->cor = 0;

            while (pilha->topo != NULL) {
                int idVertice = pop(pilha);
                vertice = busca_vertice(g, idVertice);
                TVizinho *vizinho = vertice->prim_vizinho;

                while (vizinho != NULL) {
                    TGrafo *vertice_vizinho = busca_vertice(g, vizinho->id_vizinho);
                    if (vertice_vizinho->cor == -1) {
                        push(pilha, vertice_vizinho->id_vertice);
                        vertice_vizinho->cor = 1 - vertice->cor;
                    } else if (vertice_vizinho->cor == vertice->cor) {
                        libera(pilha);
                        return 0;
                    }
                    vizinho = vizinho->prox;
