...
int percorrimento(TNo *raiz) {
    int chave_buscada = 6;

    TNo *no_encontrado = busca(raiz, chave_buscada);
    if (no_encontrado)
    {
        int position = posicao(no_encontrado, raiz);
        int keyFind = raiz->s[position];

        if (position < no_encontrado->m && keyFind == no_encontrado->s[position])
        {
            printf("Chave encontrada %d\n", keyFind);
            return keyFind;
        }
        else{
            printf("Chave não encontrada: %d a chave encontrada foi\n", chave_buscada, keyFind);
            return -1;
        }
    }
    else{
         printf("Chave não encontrada, nó nulo%d\n");
         return -1;
    }    
}
